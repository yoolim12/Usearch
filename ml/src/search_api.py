...
from ml.src.crawler import Crawler
...
class SearchAPI:
            ...
            if res_code == 200:
                ...
                if (img_url == "Nnews"):
                    for x in res_body["items"]:
                        ...
                        result.append(
                            {"title": title, "link": link, "description": description, "thumbnailUrl": thumb_url})
                    return result
                    ...
                    for x in res_body["items"]:
                        ...
                        description = self.convert_html_special_char(x['description'])
                        result.append(
                            {"title": title, "link": link, "description": description, "thumbnailUrl": img_url})
                    return result
    ...
    def naver_get_result(self, search_text):
        ...
        # kin_url = "https://openapi.naver.com/v1/search/kin.json?query" + enc_text + "&display=" + self.display

        result = list()

        blog_result = self.naver_parse_json(blog_url, "NoImage")
        cafe_result = self.naver_parse_json(cafe_url, "NoImage")
        news_result = self.naver_parse_json(news_url, "NoImage")
        ...
        try:
            ...
            if res_code == 200:
                ...
                for x in res_body["documents"]:
                    ...
                    result.append(
                        {"title": title, "link": x["url"], "description": description, "thumbnailUrl": x["thumbnail"]})
                return result
